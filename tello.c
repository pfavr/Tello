#include "tello.h"

#include <unistd.h>
#include <string.h>
#include <pthread.h>
#include <poll.h>
#include <stdio.h>
#include <sys/time.h>
#include <time.h>

uint8_t table8[] = {
    ,
};

uint16_t table16[] = {
    0x0000, 0x1189, 0x2312, 0x329b, 0x4624, 0x57ad, 0x6536, 0x74bf, 0x8c48, 0x9dc1, 0xaf5a, 0xbed3, 0xca6c, 0xdbe5, 0xe97e, 0xf8f7,
    0x1081, 0x0108, 0x3393, 0x221a, 0x56a5, 0x472c, 0x75b7, 0x643e, 0x9cc9, 0x8d40, 0xbfdb, 0xae52, 0xdaed, 0xcb64, 0xf9ff, 0xe876,
    0x2102, 0x308b, 0x0210, 0x1399, 0x6726, 0x76af, 0x4434, 0x55bd, 0xad4a, 0xbcc3, 0x8e58, 0x9fd1, 0xeb6e, 0xfae7, 0xc87c, 0xd9f5,
    0x3183, 0x200a, 0x1291, 0x0318, 0x77a7, 0x662e, 0x54b5, 0x453c, 0xbdcb, 0xac42, 0x9ed9, 0x8f50, 0xfbef, 0xea66, 0xd8fd, 0xc974,
    0x4204, 0x538d, 0x6116, 0x709f, 0x0420, 0x15a9, 0x2732, 0x36bb, 0xce4c, 0xdfc5, 0xed5e, 0xfcd7, 0x8868, 0x99e1, 0xab7a, 0xbaf3,
    0x5285, 0x430c, 0x7197, 0x601e, 0x14a1, 0x0528, 0x37b3, 0x263a, 0xdecd, 0xcf44, 0xfddf, 0xec56, 0x98e9, 0x8960, 0xbbfb, 0xaa72,
    0x6306, 0x728f, 0x4014, 0x519d, 0x2522, 0x34ab, 0x0630, 0x17b9, 0xef4e, 0xfec7, 0xcc5c, 0xddd5, 0xa96a, 0xb8e3, 0x8a78, 0x9bf1,
    0x7387, 0x620e, 0x5095, 0x411c, 0x35a3, 0x242a, 0x16b1, 0x0738, 0xffcf, 0xee46, 0xdcdd, 0xcd54, 0xb9eb, 0xa862, 0x9af9, 0x8b70,
    0x8408, 0x9581, 0xa71a, 0xb693, 0xc22c, 0xd3a5, 0xe13e, 0xf0b7, 0x0840, 0x19c9, 0x2b52, 0x3adb, 0x4e64, 0x5fed, 0x6d76, 0x7cff,
    0x9489, 0x8500, 0xb79b, 0xa612, 0xd2ad, 0xc324, 0xf1bf, 0xe036, 0x18c1, 0x0948, 0x3bd3, 0x2a5a, 0x5ee5, 0x4f6c, 0x7df7, 0x6c7e,
    0xa50a, 0xb483, 0x8618, 0x9791, 0xe32e, 0xf2a7, 0xc03c, 0xd1b5, 0x2942, 0x38cb, 0x0a50, 0x1bd9, 0x6f66, 0x7eef, 0x4c74, 0x5dfd,
    0xb58b, 0xa402, 0x9699, 0x8710, 0xf3af, 0xe226, 0xd0bd, 0xc134, 0x39c3, 0x284a, 0x1ad1, 0x0b58, 0x7fe7, 0x6e6e, 0x5cf5, 0x4d7c,
    0xc60c, 0xd785, 0xe51e, 0xf497, 0x8028, 0x91a1, 0xa33a, 0xb2b3, 0x4a44, 0x5bcd, 0x6956, 0x78df, 0x0c60, 0x1de9, 0x2f72, 0x3efb,
    0xd68d, 0xc704, 0xf59f, 0xe416, 0x90a9, 0x8120, 0xb3bb, 0xa232, 0x5ac5, 0x4b4c, 0x79d7, 0x685e, 0x1ce1, 0x0d68, 0x3ff3, 0x2e7a,
    0xe70e, 0xf687, 0xc41c, 0xd595, 0xa12a, 0xb0a3, 0x8238, 0x93b1, 0x6b46, 0x7acf, 0x4854, 0x59dd, 0x2d62, 0x3ceb, 0x0e70, 0x1ff9,
    0xf78f, 0xe606, 0xd49d, 0xc514, 0xb1ab, 0xa022, 0x92b9, 0x8330, 0x7bc7, 0x6a4e, 0x58d5, 0x495c, 0x3de3, 0x2c6a, 0x1ef1, 0x0f78,
};

int crc_8(uint8_t message[], int length)
{
	int i = 0;
	int crc = 0x77;
	for (i = 0; i < length; i++) crc = table8[(crc ^ message[i]) & 0xFF];
	return crc;
}

int crc_16(uint8_t message[], int length)
{
	int i = 0;
	int crc = 0x3692;
	for (i = 0; i < length; i++) crc = table16[(crc ^ message[i]) & 0xFF] ^ (crc >> 8);
	return crc;
}

void send_package(struct tello * tello, int type, int id, int sequence, int* data, int datasize)
{
	int packagesize = 11 + datasize;
	uint8_t package[packagesize];
	int start = 204;
	int size = packagesize * 8;
	memcpy(&package[0], &start, 1);
	memcpy(&package[1], &size, 2);
	int crc8 = crc_8(package, 3);
	memcpy(&package[3], &crc8, 1);
	memcpy(&package[4], &type, 1);
	memcpy(&package[5], &id, 2);
	memcpy(&package[7], &sequence, 2);
	for (int i = 0; i < datasize; i++) memcpy(&package[9+i], &data[i], 1);
	int crc16 = crc_16(package, 9+datasize);
	memcpy(&package[9+datasize], &crc16, 2);
	sendto(tello->data_socket, package, packagesize, 0, (struct sockaddr *)&tello->address, sizeof(tello->address));
}

void *poll_thread(void *ptr)
{
	struct tello * tello = (struct tello *)ptr;
	struct timespec last_keepalive = {0};
	while (tello->data_socket > 0) {
		long a1 = tello->speed_mode;
		long a2 = tello->left_x * 660 + 1024;
		long a3 = tello->left_y * 660 + 1024;
		long a4 = tello->right_y * 660 + 1024;
		long a5 = tello->right_x * 660 + 1024;
		long packedAxis = (a1 << 44) | (a2 << 33) | (a3 << 22) | (a4 << 11) | a5;
		struct timespec ts;
	    	clock_gettime(CLOCK_REALTIME, &ts);
		struct tm *lt = localtime(&ts.tv_sec);
		int msec = ts.tv_nsec/1000000;
		int data[] = {
			packedAxis & 0xFF, packedAxis >> 8 & 0xFF, packedAxis >> 16 & 0xFF,
			packedAxis >> 24 & 0xFF, packedAxis >> 32 & 0xFF, packedAxis >> 40 & 0xFF,
			lt->tm_hour, lt->tm_min, lt->tm_sec, msec & 0xFF, msec >> 8 & 0xFF
		};
		send_package(tello, 96, 80, 0, data, 11);
		struct timespec now;
		clock_gettime(CLOCK_MONOTONIC, &now);
		long keepalive_ms = (now.tv_sec - last_keepalive.tv_sec) * 1000
			+ (now.tv_nsec - last_keepalive.tv_nsec) / 1000000;
		if (last_keepalive.tv_sec == 0 || keepalive_ms >= 5000) {
			send_package(tello, 72, 69, tello->sequence++, NULL, 0);
			last_keepalive = now;
		}
		struct timespec delay = {.tv_nsec = 20000000};
		nanosleep(&delay, NULL);
	}
	return NULL;
}

void *data_thread(void *ptr)
{
	struct tello * tello = (struct tello *)ptr;
	uint8_t package[1012];
	while (tello->data_socket > 0) {
		int received = recv(tello->data_socket, package, sizeof(package), 0);
		if (received < 11) continue;
		int size = (package[1] | package[2] << 8) / 8;
		if (size < 11 || size > received) continue;
		int crc8 = package[3];
		if (crc8 != crc_8(package, 3)) continue;
		int id = package[5] | package[6] << 8;
		int datasize = size - 11;	
		int crc16 = package[9+datasize] | package[10+datasize] << 8;
		if (crc16 != crc_16(package, 9+datasize)) continue;
		switch (id) {
			case 69: {
				if (!tello->keepalive_acknowledged)
					fprintf(stderr, "Tello keepalive acknowledged.\n");
				tello->keepalive_acknowledged = 1;
				if (tello->data_callback) (*tello->data_callback)(id);
				continue;
			}
			case 26: {
				tello->wifi_strength = package[9];
				tello->wifi_disturb = package[10];
				if (tello->data_callback) (*tello->data_callback)(id);
				continue;
			}
			case 53: {
				tello->light_strength = package[9];
				if (tello->data_callback) (*tello->data_callback)(id);
				continue;
			}
			case 70: {
				struct timespec ts;
			    	clock_gettime(CLOCK_REALTIME, &ts);
				struct tm *lt = localtime(&ts.tv_sec);
				int msec = ts.tv_nsec/1000000;
				int senddata[] = {
					lt->tm_year & 0xFF, lt->tm_year >> 8 & 0xFF,
					lt->tm_mon & 0xFF, lt->tm_mon >> 8 & 0xFF,
					lt->tm_mday & 0xFF, lt->tm_mday >> 8 & 0xFF,
					lt->tm_hour & 0xFF, lt->tm_hour >> 8 & 0xFF,
					lt->tm_min & 0xFF, lt->tm_min >> 8 & 0xFF,
					lt->tm_sec & 0xFF, lt->tm_sec >> 8 & 0xFF,
					msec & 0xFF, msec >> 8 & 0xFF
				};
				send_package(tello, 80, 70, tello->sequence++, senddata, 12);
				continue;
			}
			case 86: {
				memcpy(&tello->height, &package[9], 2);
				memcpy(&tello->north_speed, &package[11], 2);
				memcpy(&tello->east_speed, &package[13], 2);
				memcpy(&tello->ground_speed, &package[15], 2);
				memcpy(&tello->fly_time, &package[17], 2);
				tello->imu_state = package[19] >> 0 & 1;
				tello->pressure_state = package[19] >> 1 & 1;
				tello->visual_state = package[19] >> 2 & 1;
				tello->power_state = package[19] >> 3 & 1;
				tello->battery_state = package[19] >> 4 & 1;
				tello->gravity_state = package[19] >> 5 & 1;
				tello->wind_state = package[19] >> 7 & 1;
				tello->imu_calibration_state = package[20];
				tello->battery_percentage = package[21];
				memcpy(&tello->fly_time_left, &package[22], 2);
				memcpy(&tello->battery_left, &package[24], 2);
				tello->sky = package[26] >> 0 & 1;
				tello->ground = package[26] >> 1 & 1;
				tello->open = package[26] >> 2 & 1;
				tello->hover = package[26] >> 3 & 1;
				tello->outage_recording = package[26] >> 4 & 1;
				tello->battery_low = package[26] >> 5 & 1;
				tello->battery_lower = package[26] >> 6 & 1;
				tello->factory_mode = package[26] >> 7 & 1;
				tello->fly_mode = package[27];
				tello->throw_fly_timer = package[28];
				tello->camera_state = package[29];
				tello->electrical_machinery_state = package[30];
				tello->front_in = package[31] >> 0 & 1;
				tello->front_out = package[31] >> 1 & 1;
				tello->front_lsc = package[31] >> 2 & 1;
				tello->temperature_height = package[32] >> 0 & 1;
				if (tello->data_callback) (*tello->data_callback)(id);
				continue;
			}
			case 4176: {
				int senddata[] = {0, package[9], package[10]};
				send_package(tello, 80, 4176, tello->sequence++, senddata, 3);
				continue;
			}
			case 4177: {
				int position = 10;
				while (position < size - 2) {
					if (package[position + 0] != 85) break;
					int chunkSize = package[position + 1];
					if (package[position + 2] != 0) break;
					int crc8 = package[position + 3];
					if (crc8 != crc_8(&package[position], 3)) break;
					int id = package[position + 4] | package[position + 5] << 8;
                   			uint8_t decrypt = package[position + 6];
					switch (id) {
						case 29: {
							for (int i = 0; i < chunkSize; i++) package[position + i] = package[position + i] ^ decrypt;
							memcpy(&tello->velocity_z, &package[position + 12], 2);
							memcpy(&tello->velocity_x, &package[position + 14], 2);
							memcpy(&tello->velocity_y, &package[position + 16], 2);
							memcpy(&tello->position_z, &package[position + 18], 4);
							memcpy(&tello->position_x, &package[position + 22], 4);
							memcpy(&tello->position_y, &package[position + 26], 4);
							memcpy(&tello->position_uncertainty, &package[position + 30], 4);
							if (tello->data_callback) (*tello->data_callback)(id);
							break;
						}
						case 2048: {
							for (int i = 0; i < chunkSize; i++) package[position + i] = package[position + i] ^ decrypt;
							memcpy(&tello->rotation_w, &package[position + 58], 4);
							memcpy(&tello->rotation_z, &package[position + 62], 4);
							memcpy(&tello->rotation_x, &package[position + 66], 4);
							memcpy(&tello->rotation_y, &package[position + 70], 4);
							memcpy(&tello->relative_velocity_z, &package[position + 86], 4);
							memcpy(&tello->relative_velocity_x, &package[position + 90], 4);
							memcpy(&tello->relative_velocity_y, &package[position + 94], 4);
							if (chunkSize >= 118) {
								int16_t raw_temp;
								memcpy(&raw_temp, &package[position + 116], 2);
								tello->imu_temperature = raw_temp / 100.0f;
							}
							if (tello->data_callback) (*tello->data_callback)(id);
							break;
						}
					}
					position += chunkSize;
				}
				continue;
			}
		}		
	}
	return NULL;
}

void *camera_thread(void *ptr)
{
	struct tello * tello = (struct tello *)ptr;
	uint8_t package[1460];
	while (tello->camera_socket > 0) {
		int size = recv(tello->camera_socket, package, 1460, 0);
		if (size > 0 && tello->camera_callback) (*tello->camera_callback)(package, size);
	}
	return NULL;
}

void tello_disconnect(struct tello* tello)
{
	int data_socket = tello->data_socket;
	int camera_socket = tello->camera_socket;
	tello->data_socket = 0;
	tello->camera_socket = 0;
	if (data_socket > 0) close(data_socket);
	if (camera_socket > 0) close(camera_socket);
	if (tello->poll_thread_started) pthread_join(tello->poll_thread, NULL);
	if (tello->data_thread_started) pthread_join(tello->data_thread, NULL);
	if (tello->camera_thread_started) pthread_join(tello->camera_thread, NULL);
	tello->poll_thread_started = 0;
	tello->data_thread_started = 0;
	tello->camera_thread_started = 0;
}

int tello_connect(struct tello* tello, int camera_port, int timeout)
{
	memset(tello, 0, sizeof(struct tello));

	tello->data_socket = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
	int reuse_address = 1;
	setsockopt(tello->data_socket, SOL_SOCKET, SO_REUSEADDR, &reuse_address, sizeof(reuse_address));
	struct sockaddr_in local_address;
	memset(&local_address, 0, sizeof(local_address));
	local_address.sin_family = AF_INET;
	local_address.sin_port = htons(9000);
	local_address.sin_addr.s_addr = htonl(INADDR_ANY);
	if (bind(tello->data_socket, (struct sockaddr *)&local_address, sizeof(local_address)) < 0) {
		tello_disconnect(tello);
		return -1;
	}

	tello->address.sin_family = AF_INET;
	tello->address.sin_port = htons(8889);
	inet_pton(AF_INET, "192.168.10.1", &(tello->address.sin_addr));

	char req_package[12];
	memcpy(&req_package[0], "conn_req:", 9);
	memcpy(&req_package[9], &camera_port, 2);
	req_package[11] = '\0';

	char received_package[1012];
	sendto(tello->data_socket, req_package, 11, 0, (struct sockaddr *)&tello->address, sizeof(tello->address));

	struct timespec started;
	clock_gettime(CLOCK_MONOTONIC, &started);
	int acknowledged = 0;
	while (!acknowledged) {
		struct timespec now;
		clock_gettime(CLOCK_MONOTONIC, &now);
		long elapsed_ms = (now.tv_sec - started.tv_sec) * 1000
			+ (now.tv_nsec - started.tv_nsec) / 1000000;
		int remaining_ms = timeout * 1000 - (int)elapsed_ms;
		if (remaining_ms <= 0) break;
		struct pollfd descriptor = {.fd = tello->data_socket, .events = POLLIN};
		if (poll(&descriptor, 1, remaining_ms) <= 0) break;
		int received = recv(tello->data_socket, received_package, sizeof(received_package), 0);
		if (received >= 9 && memcmp(received_package, "conn_ack:", 9) == 0)
			acknowledged = 1;
	}
	if (!acknowledged) { tello_disconnect(tello); return -1; }
	
	struct timeval tv;
	tv.tv_sec = 0; tv.tv_usec = 200000;
	setsockopt(tello->data_socket, SOL_SOCKET, SO_RCVTIMEO, &tv, sizeof(tv));

	tello->camera_socket = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);

	struct sockaddr_in camera_address;
	memset(&camera_address, 0, sizeof(camera_address));
	camera_address.sin_family = AF_INET;
	camera_address.sin_port = htons(camera_port);
	camera_address.sin_addr.s_addr = htonl(INADDR_ANY);
	bind(tello->camera_socket, (struct sockaddr*)&camera_address, sizeof(camera_address));
	/* A large receive buffer absorbs Wi-Fi bursts and reduces the packet
	 * loss that corrupts H.264 P-frames (the main source of blocky video). */
	int camera_buffer_size = 4 * 1024 * 1024;
	setsockopt(tello->camera_socket, SOL_SOCKET, SO_RCVBUF,
		&camera_buffer_size, sizeof(camera_buffer_size));
	setsockopt(tello->camera_socket, SOL_SOCKET, SO_RCVTIMEO, &tv, sizeof(tv));

	if (pthread_create(&tello->poll_thread, NULL, poll_thread, tello) == 0)
		tello->poll_thread_started = 1;
	if (pthread_create(&tello->data_thread, NULL, data_thread, tello) == 0)
		tello->data_thread_started = 1;
	if (pthread_create(&tello->camera_thread, NULL, camera_thread, tello) == 0)
		tello->camera_thread_started = 1;
	return 0;
}

void tello_request_iframe(struct tello* tello)
{
	send_package(tello, 96, 37, 0, NULL, 0);
}

void tello_start_video(struct tello* tello)
{
	int exposure[] = {0};
	int encoder_rate[] = {1};
	int dynamic_rate[] = {0};
	tello_camera_mode(tello, 0);
	send_package(tello, 72, 52, tello->sequence++, exposure, 1);
	send_package(tello, 104, 32, tello->sequence++, encoder_rate, 1);
	send_package(tello, 104, 33, tello->sequence++, dynamic_rate, 1);
	tello_request_iframe(tello);
}

void tello_camera_mode(struct tello* tello, int value)
{
	int data[] = {value};
	send_package(tello, 104, 49, tello->sequence++, data, 1);
}

void tello_takeoff(struct tello* tello)
{
	send_package(tello, 104, 84, tello->sequence++, NULL, 0);
}

void tello_land(struct tello* tello)
{
	int data[] = {0};
	send_package(tello, 104, 85, tello->sequence++, data, 1);
}

